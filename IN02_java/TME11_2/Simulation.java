...
public class Simulation {
    ...
    public void lance(int nbEtapes){
        ...
        try{
            int newx = agent.getX() + ((Math.random()<0.5) ? 1 : ((Math.random()<0.5) ? 0 : -1));
            int newy = agent.getY() + ((Math.random()<0.5) ? 1 : ((Math.random()<0.5) ? 0 : -1));
            ...
            if (Math.random() < 1) { // modifier le % en fonction des besoins
                System.out.println("Les gardiens se sont téléportés !");
                // téléporte tous les gardiens si le pourcentage est atteint
                for (Contenu contenu : tab) {
                    if (contenu instanceof Gardien) {
                        try {
                            ((Gardien) contenu).seTeleporter(grille);
